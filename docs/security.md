...
Room state is memory-only. Hangup removes the room and closes both signaling sockets; expiry also removes it. Restarting the signaling service invalidates all room invitations. The backend has no chat history or user-account database. Chat history and outgoing queues are stored only in each device’s browser, and optional exported backups are password-encrypted files; see [device chat](device-chat.md). Ending a room does not delete the device-local history.
